...
<body class="bg-light">
			...
			<c:choose>
								...
								<c:choose>
									...
									<c:otherwise>
										<div class="alert alert-warning m-3">
											<i class="bi bi-exclamation-triangle me-2"></i> Video chưa có
											link YouTube
										</div>
									</c:otherwise>
								</c:choose>
									...
									<div class="col-md-4">
										<div class="stat-box">
											<i class="bi bi-heart-fill text-danger"
												style="font-size: 2rem;"></i>
											<h4 class="mt-2 mb-0">${likeCount}</h4>
											<small class="text-muted">Lượt thích</small>
										</div>
									</div>
									<div class="col-md-4">
										<div class="stat-box">
											<i class="bi bi-share-fill text-success"
												style="font-size: 2rem;"></i>
								...
								<div class="d-flex gap-2 mb-4">
									<c:choose>
										<c:when test="${not empty sessionScope.currentUser}">
											...
											<!-- Nút Share (luôn enable khi đã login) -->
						...
						<div class="col-lg-4">
							<div class="suggested-section">
								<div class="info-card">
									<div
										...
										<h5 class="fw-bold mb-0">
											<i class="bi bi-collection-play me-2"></i> Video đề xuất
										</h5>
										<button class="btn btn-sm btn-outline-primary"
											onclick="location.reload()"
											title="Tải lại để xem video đề xuất khác">
									...
									<c:if test="${not empty suggestedVideos}">
										<div class="alert alert-success alert-sm mb-3 py-2">
											<i class="bi bi-check-circle"></i> <small>Đã tải
												${suggestedVideos.size()} video ngẫu nhiên</small>
										</div>
									</c:if>
									...
									<c:choose>
										<c:when test="${empty suggestedVideos}">
											<div class="alert alert-info text-center py-4">
												...
												<p class="mb-0">
													<small>Không có video đề xuất</small>
												</p>
											</div>
										</c:when>
										<c:otherwise>
											<div class="suggested-videos-list">
												<c:forEach items="${suggestedVideos}" var="sv"
													varStatus="status">
													<div class="suggested-video-card d-flex p-2 mb-2"
														...
														<div style="flex-shrink: 0;">
															<img src="${sv.poster}"
																class="suggested-video-img rounded" alt="${sv.title}"
																onerror="this.src='https://via.placeholder.com/120x70?text=No+Image'">
														</div>

														<!-- Info -->
														<div
															class="flex-grow-1 ps-3 d-flex flex-column justify-content-center">
															<div class="suggested-video-title mb-1">
																${sv.title}</div>
															<div class="d-flex align-items-center gap-3">
																<small class="text-muted"> <i
																	class="bi bi-eye-fill"></i> ${sv.views}
																</small> <small class="text-muted"> <code
																		style="font-size: 0.75rem;">${sv.id}</code>
																</small>
															</div>
														</div>
													</div>
								...
								<div class="info-card mt-3">
									...
									<img src="${video.poster}" class="img-fluid rounded"
										alt="${video.title}"
				...
				<c:otherwise>
					<div class="alert alert-danger">
						<i class="bi bi-exclamation-triangle me-2"></i> Không tìm thấy
						video!
					</div>
					<a href="<%=request.getContextPath()%>/home"
						class="btn btn-primary"> <i class="bi bi-arrow-left me-2"></i>Quay
						lại trang chủ
					</a>
				</c:otherwise>
			</c:choose>
    ...
    function redirectToLogin() {
        if (confirm('Bạn cần đăng nhập để sử dụng tính năng này. Đăng nhập ngay?')) {
            ...
            window.location.href = '<%=request.getContextPath()%>/login?returnUrl=' + encodeURIComponent(currentUrl);
        }
    }
